...
must_haves:
  truths:
    ...
    - "panels.ts defines HAPPY_PANELS, HAPPY_MAP_LAYERS, HAPPY_MOBILE_MAP_LAYERS inline and the ternary chains select them when SITE_VARIANT === 'happy'"
    - "variants/happy.ts exports DEFAULT_PANELS, DEFAULT_MAP_LAYERS, MOBILE_DEFAULT_MAP_LAYERS, and VARIANT_CONFIG for the variant config system"
...
<task type="auto">
  ...
  <done>
    ...
    - `src/config/variants/happy.ts` exports VARIANT_CONFIG, DEFAULT_PANELS, DEFAULT_MAP_LAYERS, MOBILE_DEFAULT_MAP_LAYERS (for the variant config system)
    - `src/config/panels.ts` defines HAPPY_PANELS, HAPPY_MAP_LAYERS, HAPPY_MOBILE_MAP_LAYERS inline and the ternary export chains select them when SITE_VARIANT === 'happy'
